...
def is_valid(pas: str):
	index_range = pas.find('-')
	index_char = pas.find(' ')
	start = pas.find(':') + 1
	minn = int(pas[:index_range:])
	maxx = int(pas[index_range + 1:index_char:])
	ch = pas[index_char + 1]
	count = pas[start:].count(ch)
	return minn <= count <= maxx
